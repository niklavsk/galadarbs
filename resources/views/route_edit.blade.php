...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <h4 class="card-header">{{ __('messages.Edit_route') }}</h4>
                ...
                <div class="card-body">
                    <form action="{{action('RouteController@update', ['id' => $route->id])}}" method="post">
                        ...
                        @for($i = 1; $i <= $kartasNr; $i++)
                            ...
                            <div class="form-group row {{ $empty == true || $duplicate == true ? 'has-error' : ''}}">
                                <label for="{{ $i }}" class="control-label text-md-right col-md-4">
                                    {{ trans_choice('messages.Station', $i, ['num' => $i]) }}</label>
                        ...
                        <div class="container-md text-md-center">
                            <a href="{{ action('RouteController@addStop_edit', $route->id) }}" class="btn btn-primary">
                                {{ __('messages.Add_station') }}</a>
                            <a href="{{ action('RouteController@removeStop_edit', $route->id) }}"
                               class="btn btn-secondary">{{ __('messages.Delete_station') }}</a>
                        </div>
                        <hr/>
