    ...
    <form action="{{action('DepotController@update', ['id' => $depot->id])}}" method="post">
        ...
        <div class="col-2">
            ...
            <div class="form-group {{ $errors->has('apraksts') ? 'has-error' : ''}}">
                ...
                <input class="form-control {{$errors->has('apraksts') ? ' is-invalid' : '' }}" name="apraksts" type="text" id="apraksts" value="{{ $depot->apraksts }}">
            ...
            <div class="form-group {{ $errors->has('epasts') ? 'has-error' : ''}}">
                ...
                <input class="form-control {{$errors->has('epasts') ? ' is-invalid' : '' }}" name="epasts" type="text" id="epasts" value="{{ $depot->epasts }}">
            ...
            <div class="form-group {{ $errors->has('kontakttalrunis') ? 'has-error' : ''}}">
                ...
                <input class="form-control {{$errors->has('kontakttalrunis') ? ' is-invalid' : '' }}" name="kontakttalrunis" type="text" id="kontakttalrunis" value="{{ $depot->kontakttalrunis }}">
            ...
            <div class="form-group {{ $errors->has('depo_vaditajs') ? 'has-error' : ''}}">
                ...
                <select name="depo_vaditajs" size="1" class="form-control {{$errors->has('depo_vaditajs') ? ' is-invalid' : '' }}" id="depo_vaditajs">
                    ...
                    @foreach($directors as $director)
                        <option value="{{ $director->director_id }}" @if($depot->depo_vaditajs == $director->director_id) selected @endif>{{ $director->vards }} {{ $director->uzvards }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('atrasanas_vieta') ? 'has-error' : ''}}">
                ...
                <select name="atrasanas_vieta" size="1" class="form-control {{$errors->has('atrasanas_vieta') ? ' is-invalid' : '' }}" id="atrasanas_vieta">
                    ...
                    @foreach($addresses as $address)
                        <option value="{{ $address->id }}" @if($depot->atrasanas_vieta == $address->id) selected @endif>
