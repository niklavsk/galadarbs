...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <h4 class="card-header">Pievienot maršrutu</h4>
                <div class="card-body">
                    <form action="{{action('RouteController@store')}}" method="post">
                        ...
                        @for($i = 1; $i <= $kartasNr; $i++)
                            ...
                            <div class="form-group row {{ $empty == true || $duplicate == true ? 'has-error' : ''}}">
                                <label for="{{ $i }}" class="control-label text-md-right col-md-4">{{ $i }}. pietura</label>
                        ...
                        <div class="container-md text-md-center">
                            <a href="{{ action('RouteController@addStop') }}" class="btn btn-primary">Pievienot pieturu</a>
                            <a href="{{ action('RouteController@removeStop') }}" class="btn btn-secondary">Noņemt pieturu</a>
                        </div>
                        <hr/>
